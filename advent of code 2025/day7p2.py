    ...
    for c in range(cols):
        if matrix[r][c] == "|":
            ...
            if r == 0 or matrix[r-1][c] != "|":
                if c == 0:
                    # At left edge - count +2 if there's a ^ to the right (one path continues, one goes off edge)
                    if c + 1 < cols and matrix[r][c+1] == "^":
                        count += 2

                elif c == cols - 1:
                    # At right edge - count +2 if there's a ^ to the left (one path continues, one goes off edge)
